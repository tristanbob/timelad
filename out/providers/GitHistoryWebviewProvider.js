...
exports.GitHistoryWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const GitService_1 = require("../services/GitService");
const NotificationService_1 = require("../services/NotificationService");
const FileOperationsService_1 = require("../services/FileOperationsService");
const webviewTemplates_1 = require("../views/templates/webviewTemplates");
const constants = __importStar(require("../constants"));
// Helper functions to convert between types
function commitDataToGitCommit(commit) {
    return {
        hash: commit.hash,
...
class GitHistoryWebviewProvider {
    ...
    async hasUnsavedChanges() {
        const unsavedDocuments = vscode.workspace.textDocuments.filter(doc => doc.isDirty && !doc.isUntitled);
    ...
    async restoreVersion(commitHash, skipConfirmation = false) {
        ...
        try {
            ...
            if (this.view) {
                ...
                this.view.webview.html = (0, webviewTemplates_1.getSidebarTemplate)(commits, this.uncommittedChanges);
            }
            statusBar.dispose();
        }
        catch (error) {
    ...
    async createCommitDetailsPanel(commit, commitDetails) {
        ...
        const panel = vscode.window.createWebviewPanel(constants.COMMIT_DETAILS_VIEW_ID, `${constants.EXTENSION_NAME}: Version ${commit.version || commit.hash}`, vscode.ViewColumn.One, {
            enableScripts: false,
    ...
    async discardChanges(skipConfirmation = false) {
        try {
            ...
            if (!skipConfirmation) {
                ...
                const shouldDiscard = await this.notificationService.showDestructiveConfirmation('You have uncommitted changes that will be discarded:', fileNames, 'Discard Changes', 'Cancel');
                if (!shouldDiscard) {
                    return;
                }
            }
    ...
    async saveChanges(options = {}) {
        ...
        try {
            ...
            // Show success message
            if (this.view) {
                this.view.webview.html = this.getSuccessTemplate("Changes saved!", `"${commitMessage}"`);
                setTimeout(async () => {
                    await this.refresh();
                }, 3000);
            }
            else {
                await this.notificationService.showInfo(`${constants.MESSAGES.CHANGES_SAVED}\nCommit: "${commitMessage}"`);
                await this.refresh();
            }
        }
        catch (error) {
    ...
    async createRepository() {
        try {
            if (this.view) {
                this.view.webview.html = (0, webviewTemplates_1.getLoadingTemplate)();
            }
    ...
    async refresh() {
        ...
        try {
            ...
            this.view.webview.html = (0, webviewTemplates_1.getSidebarTemplate)(this.commits, this.uncommittedChanges, paginationInfo);
        }
        catch (error) {
...
exports.GitHistoryWebviewProvider = GitHistoryWebviewProvider;
//# sourceMappingURL=GitHistoryWebviewProvider.js.map