...
const commonJavaScript = `
  ...
  function viewCommit(hash) {
    // Find the commit item and extract version from it
    const commitItem = document.querySelector('[data-hash="' + hash + '"]');
    let version = null;
    if (commitItem) {
      const versionElement = commitItem.querySelector('.commit-version');
      if (versionElement) {
    ...
    vscode.postMessage({ command: 'showCommit', hash: hash, version: version });
  }
  ...
  function generateCommitItemHTML(commit, index) {
    return \`
      <div class="commit-header">
          <span class="commit-version-large">Version \${commit.version}</span>
      </div>
  ...
  document.addEventListener('click', function(event) {
    if (event.target.classList.contains('commit-version') || 
        event.target.classList.contains('version-badge')) {
  ...
  return `
    ...
    <body>
        <div class="alpha-warning">
            ...
            <span class="alpha-warning-text">BETA VERSION - Testing Only - Use with Disposable Data</span>
        </div>
    ...
    <div class="uncommitted-section">
      <div class="uncommitted-header">
        ...
        <div class="uncommitted-actions">
          ...
          <button class="save-btn" onclick="saveChanges()" title="Save changes with auto-generated commit message">
            💾 Save
          </button>
        </div>
      </div>
 ...
 */
function getCommitDetailsTemplate(commit, commitDetails, version = null) {
  // Use passed version or fallback to commit.version or default
  const displayVersion = version || commit.version || 'Details';
  ...
  return `
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Version ${displayVersion}</title>
        <style>
            ${baseStyles}
            ${commitDetailsStyles}
            body {
    ...
    <body>
        <h1><span class="version-badge">Version ${displayVersion}</span></h1>
